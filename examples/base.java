...
public class base {
    ...
    private static int jkl=0;
    private static int p=0;
    private static Map<Integer, Integer> indexValueMap = new HashMap<>();
        ...
        try {
         ...
         // Access the list of hosts from the datacenter
            ...
            while (finishedCloudlets < cloudletList.size()) {
             ...
             //   calculateAndAddTemperatureChange(cloudletList.subList(finishedCloudlets, toIndex), tempeeratureModel, vmii);               
   ...
   private static void saveserverdata() {
	   String csvFilePath = "serverscore.csv";
	   try (CSVWriter writer = new CSVWriter(new FileWriter(csvFilePath))) {
           // If it's the first write, clear the file and write the header
      	/*if (now == 0) {
              writer.write("Server ID,");
              writer.write("Number of Cores,");
              for (int i = 1; i <= 1023; i++) {
                  writer.write("ser" + i + ",");
              }
              //writer.write("\n");
              now = 1; // Set the flag to indicate that the file has been cleared
              writer.write("\n");
          }
*/
      	 for (Map.Entry<Integer, Integer> entry : indexValueMap.entrySet()) {
             String[] data = {entry.getKey().toString(), entry.getValue().toString()};
    ...
    private static void calculateAndAddTemperatureChange(Cloudlet cloudlet, TempeeratureModel tempeeratureModel, int vmId, double start,double finish, int cores) {
    	try (FileWriter writer = new FileWriter("temperaturesbybase.csv", true)) {
            // If it's the first write, clear the file and write the header
    		int serverId = vmId;
       	if (!file2Cleared == true) {
               ...
               writer.write("Cores Used,");
               writer.write("Server Assigned,");
               writer.write("Server cores,");
               for (int i = 1; i <= 1023; i++) {
                   writer.write("ser" + i + ",");
               }
               //writer.write("\n");
    ...
    //	for (Cloudlet cloudlet : cloudlets) {
            ...
            if(serverId>991) {
                ...
                writer.write( (serverId+1) + ","); 
                writer.write( indexValueMap.get(serverId) + ",");
                for (Vm vm : vmList) {
                	 int sId = vm.getId();
                	//System.out.println(serverId);
    ...
    private static List<Cloudlet> createCloudletsFromCSV(int brokerId, String csvFile) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                cloudlet.setVmId(getVmIdForCloudlet(cloudlet, tempeeratureModel,startTime,finishTime,cpusUsed));
                //now++;
    ...
    private static int getVmIdForCloudlet(Cloudlet cloudlet, TempeeratureModel tempeeratureModel, double start, double finish, int cpusUsed) {
    	// vmId=0;
        int cloudletId = cloudlet.getCloudletId();
        // Find the VM with the lowest temperature
        int vmIdWithLowestTemp = -1;
        double lowestTemperature = Double.MAX_VALUE;

        for (Vm vm : vmList) {
            int vmId = vm.getId();
            double temperature = tempeeratureModel.getTemperature(vmId);

            if (temperature < lowestTemperature) {
                lowestTemperature = temperature;
                vmIdWithLowestTemp = vmId;
            }
        }

        // Update the temperature of the selected VM based on the cloudlet's execution
       //tempeeratureModel.updateTemperature(vmIdWithLowestTemp, temperatureChange);
        
        if(jkl==0) {
        	jkl=1;
        for (int i = 0; i < 1023; i++) {
            indexValueMap.put(i, 0); // Initial value is set to 0
        }
        }
        System.out.println(cpusUsed);
        int currentValue = indexValueMap.get(vmIdWithLowestTemp);
        int updatedValue = currentValue + cpusUsed;

        // Update the value in the list
        indexValueMap.put(vmIdWithLowestTemp, updatedValue);

        // Display the updated list
        System.out.println("Updated List: " + indexValueMap);
        calculateAndAddTemperatureChange(cloudlet , tempeeratureModel, vmIdWithLowestTemp, start, finish,cpusUsed);
        
        saveserverdata();
        return vmIdWithLowestTemp;
    }
